...
public class ExcelReader {
  ...
  try {
   ...
   if (row.getCell((short) cellNum) != null) { // add this condition
    ...
    case HSSFCell.CELL_TYPE_NUMERIC: {
     strExcelCell = new BigDecimal(row.getCell((short) cellNum).getNumericCellValue()).toPlainString();
    }
     break;
 ...
 public static String[][] read(File file)
 {
	  ...
	  for (int i = 0; i <= count; i++) {
		  value[i] = readExcel.readExcelLine(i);
	  }
	  return value;
 }
 //主函数用于测试
 public static void main(String args[]) {
  //File file = new File("/home/snow/abb.xls");
