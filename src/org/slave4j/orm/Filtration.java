...
public class Filtration
{
	...
	public static final int FilterOn = 1;
	public static final int FilterOff = 0;// 1 代表可以支持多条件匹配
	...
	private int choice = Filtration.FilterOff;
	
	private MatchType[] mTypes =null;
	
	/**
	 * 构造方法(主要用于页面传值，并由WebUtils调用)
	 * @param filterName 比较属性字符串,含待比较的比较类型、属性值类型及属性列表. 如： LIKES_NAME_OR_EQL_parentId
	 * @param filterValue 待比较的值.
	 * @param choice 1 代表 组合查询，比较值变化
	 */
	public Filtration(String filterName, String filterValue,int choice)
	{
	
		String[] fitStrings=filterName.split(Filtration.OR);
		mTypes=new MatchType[fitStrings.length];
		fieldNames=new String[fitStrings.length];
		for(int i=0;i<fitStrings.length;i++){
			String temp1=fitStrings[i];
			String condition = StringUtils.substringBefore(temp1, "_");//条件
			...
			mTypes[i] = Enum.valueOf(MatchType.class, matchTypeCode);
			fieldType = Enum.valueOf(FiledType.class, fieldTypeCode).getValue();
			String fieldName1 = StringUtils.substringAfter(temp1, "_");
			fieldNames[i]=fieldName1;
	...
	public MatchType[] getMTypes() {
		return mTypes;
	}


	public void setMTypes(MatchType[] matchTypes) {
		this.mTypes = matchTypes;
	}
