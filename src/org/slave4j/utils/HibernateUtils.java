...
public class HibernateUtils
{
	...
	private static Criterion createCriterion(String fieldName, Object fieldValue, MatchType matchType)
	{
		...
		case EQ: // =
			...
			break;
		case LIKE: // like
			criterion = Restrictions.like(fieldName, String.valueOf(fieldValue), MatchMode.ANYWHERE);
			break;
		case LT: // <
		...
		if (filtrations.length > 0)
		{
			...
			for (Filtration filtration : filtrations)
			{
				...
				} else
				{
					...
					if(filtration.getChoice()==Filtration.FilterOff){
						...
						if(filtration.getChoice()==Filtration.FilterOn){
							String[] filedNames=filtration.getFieldNames();
							 MatchType[] matTypes=filtration.getMTypes();
							String filedName;
							for(int m=0;m<filedNames.length;m++){
								filedName=filedNames[m];
								criterion = createCriterion(filedName, filtration.getFieldValue(), matTypes[m]);
								disjunction.add(criterion);
							}
						}
					}
