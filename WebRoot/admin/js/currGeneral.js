Ext.onReady(function(){
	...
	var selectedLine = ['rtV'];


	var hashKeyArr = ['xdate','week','edition','serverId','currType','putV','outV','rtV']
	var fieldNames = ['货币类型','产出量','消耗量','留存量'];
	var keyName = [];
	var hashkeyMap = {};
	...
	toolbar.add("开始日期",
				 ...
				 new Ext.Button({
						...
						handler:function(){
							...
							var param = {start:start,end:end, edition:edition, serverId:serverId, currType:currType};
							var url='currAnalyze.do?method=generalData&target=excel&' + Ext.urlEncode(param);
							window.open(url);
						}
				 })
	); 
	...
	store = new Ext.data.Store({
		proxy:new Ext.data.HttpProxy({url:'currAnalyze.do?method=generalData'}),
...
//	var parseNanRender = function(value){if(isNaN(value)) return "-"; return value}
	window.kkk = store;
