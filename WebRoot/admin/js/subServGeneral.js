Ext.onReady(function(){
	...
	var selectedLine = ['ActiveUserCount'];
	...
	var columnStart = [//对应的列
     ...
     {header:'累计收入',width:150,dataIndex:'SubServTotalFee',summaryType: 'sum',sortable:true},
     {header:'创角用户数',width:150,dataIndex:'CreateUserCount',summaryType: 'sum',sortable:true},
     {header:'累计创角用户数',width:150,dataIndex:'SubServTotalCUC',summaryType: 'sum',sortable:true},
	...
	if(window.mLevel >= 2){
	    ...
	    {header:'累计付费用户数',width:150,dataIndex:'SubServTotalPUC',summaryType: 'sum',sortable:true},
	    {header:'付费Arppu值',width:150,dataIndex:'SubServArppu',sortable:true},
	    {header:'付费Arpu值',width:150,dataIndex:'SubServArpu',sortable:true},
	    {header:'付费渗透率',width:150,dataIndex:'SubServPUP',sortable:true,  renderer:percentRender},
	    {header:'ACU',width:150,dataIndex:'SubServACU',sortable:true},
	    {header:'PCU',width:150,dataIndex:'SubServPCU',sortable:true},
	    {header:'ACU/PCU',width:150,dataIndex:'ACUPCUCal',sortable:true}
    )}
	var keyName = [];
	var hashkeyMap = {};
				...
				'-',
				 new Ext.Button({
						...
						handler:function(){
							...
							var url='subServAnalyze.do?method=generalData&target=excel&' + Ext.urlEncode(param);
							window.open(url);
						}
				 }))
	}
	...
	store = new Ext.data.Store({
		proxy:new Ext.data.HttpProxy({url:'subServAnalyze.do?method=generalData'}),
