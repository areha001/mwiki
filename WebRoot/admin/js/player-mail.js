		...
		var lockWindow = new Ext.Window({
			...
			buttons: [{
				...
				handler:function(){
					...
					params:{serverId:user.sid, roleId:user.roleId},
					success:function(form,action){
					window.Ext.Msg.alert("提示","操作成功！",function(){
						storeUser.load({
							params:{"searchUserType":1, "searchUserName":user.userName,"serverId":user.sid}
						});
						lockWindow.hide();
					});
				},
	...
	var lockPageHistory = function(user){
		...
		var grid = new Ext.grid.GridPanel({
			...
			columns:[//对应的列
			         ...
			         {header:'封停至 ',width:150,dataIndex:'lockTo',sortable:true,renderer:function(value, metadata,record)
		        		 {
		        		 var status = record.data.lockTo;
		        		 if(status == "-1"){return "<font color='red'>永久封号</font>"}
		        		 if(status == "0"){return "<font color='red'>恢复正常</font>"}
		        		 var lockTo = status -0;
		        		 return Ext.util.Format.date(new Date(lockTo),'Y-m-d  H:i')
	        		 }},
