...
Ext.onReady(function(){
	...
	function exportExcel(){
		...
		var url='mallBuy.do?method=findMallBuyList&target=excel&' + Ext.urlEncode(params);
		window.open(url);	
	}
...
//	   autoLoad:true,
		...
		proxy : new Ext.data.HttpProxy({
			url:'mallBuy.do?method=findMallBuyList'//点击进入
			//baseParams:{hello:1}
			//alert();
		})
...
//			"parnter":Ext.getCmp('parnterid').getValue(), start:0,limit:25}
	...
	var grid = new Ext.grid.GridPanel({
		...
		columns:[
				{header:'购买玩家ID',width:130,dataIndex:'playerid',sortable:true},
				{header:'购买玩家名',width:130,dataIndex:'name',sortable:true},
				{header:'服务器',width:130,dataIndex:'serverid',sortable:true},
				{header:'消费钻石',width:130,dataIndex:'diamond',sortable:true},
				{header:'购买物品',width:130,dataIndex:'goodname',sortable:true},
				{header:'数量',width:130,dataIndex:'num',sortable:true},
	            {
	                header: '商城类型 ', width: 130, dataIndex: 'goodType', sortable: true,
