...
Ext.onReady(function(){
	...
	function exportExcel(){
		...
		var url='average.do?method=findAverage&target=excel&' + Ext.urlEncode(params);
		window.open(url);	
	}
...
//	   autoLoad:true,
		...
		proxy : new Ext.data.HttpProxy({
			url:'average.do?method=findAverage'//点击进入
			//baseParams:{hello:1}
			//alert();
		})
...
//	var outcolums = new Ext.grid.ColumnModel(outcolumnArr);
	...
	var grid = new Ext.grid.GridPanel({
		...
		columns:[
				{header:'时间',width:'10%',dataIndex:'date',sortable:true},
				{header:'人均拥有钻石',width:130,dataIndex:'own',sortable:true},
				{header:'人均消费钻石',width:130,dataIndex:'consume',sortable:true},
				{header:'登陆人数',width:130,dataIndex:'amount',sortable:true}
		   ]
	});
	window.top.ee = grid;
	new Ext.Viewport({
