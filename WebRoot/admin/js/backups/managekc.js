 ...
 Ext.onReady(function(){
			        ...
			        buttons: [{
			            ...
			            handler:function(){
				        	dataform.form.submit({
				    			...
				    			waitMsg:'正在添加信息,请稍后...',
				    			waitTitle:'提示',
				    			url:"main.do?method=addkc",   // <-这里输入添加教师信息的servlet地址。   return null out.print({"success":false});
	...
	toolbar.add(
		...
		new Ext.Button({
			text:'添加知识点',
			iconCls:'addButton',
			handler:addKCHandler(0)
		}),
		
		new Ext.Button({
			text:'添加测试点',
			iconCls:'addButton',
			handler:addKCHandler(1)
		}),
			        ...
			        buttons: [{
			            ...
			            handler:function(){
				        	dataform.form.submit({
				    			...
				    			waitMsg:'正在添加信息,请稍后...',
				    			waitTitle:'提示',
				    			url:"main.do?method=editkc",   // <-这里输入添加教师信息的servlet地址。   return null out.print({"success":false});
				    			success:function(form,action){
				    				windowpanel.destroy();
				    				//store.reload();
		...
		new Ext.Button({
			...
			handler:function(){
				...
				window.Ext.Msg.confirm("提示","确认删除？删除将连下面的子结点一起删除", function(btn){
					...
					{
						Ext.Ajax.request({
		                    url: 'main.do?method=deleteKc', //根据id删除节点
		                    //
	...
	var tree = new Ext.tree.TreePanel({
		...
		listeners: {
            ...
            nodedrop:function(e){
            	if(e.point == "append")
            	{
					...
					msk.show();
	            	Ext.Ajax.request({
	            		url: "main.do?method=movekc",
	            		method : "post",
	            		params : {code:e.dropNode.id, descode:e.target.id},
	            		success:function(form,action){
	            			msk.hide();
	            			e.target.reload();
	            			var res = Ext.util.JSON.decode(form.responseText);
                            if (res.success)
	            				Ext.MessageBox.alert("提示","移动成功!");
	            			else
                           		Ext.Msg.alert('提示', res.info);
	            				
	            		},
	            		failure:function(form,action){
	            			msk.hide();
                            Ext.Msg.alert('提示', action.info);
		...
		loader : new Ext.tree.TreeLoader( {
         dataUrl : "main.do?method=treedata"// OrgTreeJsonData.action就是要动态载入数据的请求地址，这里请求时会提交一个参数为node的值，值为root即new Tree.AsyncTreeNode()对象的id值
	   })
	});
