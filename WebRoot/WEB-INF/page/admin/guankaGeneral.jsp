...
<head>
	...
	window.gkMap = {}
	<c:forEach var="i" items="${gkDicts}">
	gkMap["<c:out value='${i.subId}' escapeXml='true'/>"] = "<c:out value='${i.name}' escapeXml='true'/>"
	</c:forEach>

	var gkData = [];
	gkData.push(["   ","不限"]);
	for(var i in gkMap){
		gkData.push([i, i+"、"+ gkMap[i]]);
	}
	</script>
	
	<script type="text/javascript" src="js/guankaGeneral.js"></script>
</head>
<body>
</body>
</html>